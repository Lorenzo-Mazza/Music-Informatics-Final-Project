...
RUN_ID = '1001'   # 0- 15 sec samples, 1- 30 sec samples
SECTION = 'MTG_WRN'
...
network = build_wrn(input_dims=(12, 150), n=28, k=2, l1=l1_reg, l2=l2_reg, dropout=0.0)
...
history = network.fit(x=np.array(x, dtype=float), y=np.array(y, dtype=int),
                      ...
                      validation_split=split, callbacks=[EarlyStopping(patience=50),model_checkpoint_callback],
                      shuffle=True)
